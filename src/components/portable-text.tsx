...
const components: PortableTextComponents = {
  types: {
    ...
    simpleTable: ({ value }: { value: { caption?: string; rows: { _key: string; cells: string[] }[] } }) => {
      if (!value?.rows?.length) return null
      const [headerRow, ...bodyRows] = value.rows
      return (
        <figure className="my-6">
          {value.caption && (
            <figcaption className="mb-2 text-sm font-medium text-text">
              {value.caption}
            </figcaption>
          )}
    ...
    codeBlock: ({ value }: { value: { code: string; language?: string; filename?: string } }) => {
      ...
      return (
        <figure className="my-6 overflow-hidden rounded-lg border border-border bg-zinc-900">
          {label && (
