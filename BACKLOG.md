...
- [ ] **`[Traffic — authority]` PR & Backlink Sourcing System** *(consolidates the former "disavow the PBN profile" defense + "earn quality links" offense into one buildable system)* <!-- consolidated 2026-07-14 /brsg-session -->
  ...
  **Guardrails:** genuine value-first outreach only — no spam that would recreate the very profile we're disavowing; no fabricated data in pitches ([[shared/feedback_no_fabricated_data]]); the data reports are the honest hook. Effort **L (system build; ongoing operation)**.
  - <!-- 2026-07-21 /session --> **Phase 0 SHIPPED — the generator exists and has run across all 9 BRSG sites.** `~/Code/tools/brsg-link-audit` (private repo `billriceusa/brsg-link-audit`) @ `6ffc764`: Ahrefs refdomain pull → classifier (`is_spam` OR PBN name/TLD patterns OR DR<5-with-a-single-link → `review`) → per-site `whitelists/` + `blocklists/` (precedence is **whitelist > blocklist > classifier**, so a domain on both lists fails safe to protected) → Google-format `disavow.txt`. 13 unit tests pass. Fleet output: **2,155 domains to disavow across 9 sites, 31 whitelisted, 0 left in review.** Files staged at `~/Desktop/brsg-disavow-2026-07-21/` with an upload runbook. **GSC has no disavow API — upload is permanently manual (Bill's action).** Human triage of the 34 `review` domains protected 4 real ones found by fetching page titles rather than pattern-matching names (incl. `billricelinktree.com`, Bill's own property, and `mav.website`, the same company as the already-whitelisted `hiremav.com`) and force-disavowed 26 via the new `blocklists/` primitive so the triage survives regeneration.
  - ⚠️ **PIH's disavow file is SAMPLE-BASED — re-run before relying on it.** Its 28 domains come from a 40-domain slice pulled to exercise the classifier, **not** the full ~369-domain profile (the other 8 sites are full pulls). Every domain in it is genuinely toxic, so uploading it is safe — but it captures well under a tenth of the ~285-domain spam tail documented above. → next: full Ahrefs `referring-domains` pull for proinvestorhub.com → `npx tsx src/cli.ts audit proinvestorhub` → re-upload to GSC. **This is the P0 of the defense half:** the disavow half of this item is NOT yet actually satisfied for PIH, despite the fleet run.
...
- [ ] **`[Both — AEO]` Lean into AI-engine citation.** 12 sessions already from ChatGPT/Copilot; robots allows GPTBot/OAI/Perplexity. ✅ *2026-06-23: shipped reusable `KeyTakeaways` answer-block on all 3 data reports (`src/components/key-takeaways.tsx`).* ✅ *2026-06-27: extended the AEO `KeyTakeaways` block to all 6 strategy guides — their existing `keyTakeaways` field already held answer-bearing fact sentences, so swapped the bespoke "What You'll Learn" objectives box for the semantic AEO component (`src/app/(site)/guides/[slug]/page.tsx`); one template change lit up all guides.* ✅ *2026-07-17: shipped `llms.txt` (`src/app/llms.txt/route.ts` — curated fact index, pulls live report numbers) + explicit ClaudeBot/Claude-Web allow in `robots.ts` + AI-referral tracking (`src/components/ai-referral-tracker.tsx`, mounted in site layout → `ai_referral` dataLayer event so ChatGPT/Perplexity/Claude/Gemini traffic stops landing in "Direct").* **Remaining (UI-only, Bill):** in GTM/GA4, wire the `ai_referral` event into a custom "AI Assistant" channel + mark it; consider clean canonical markdown per report. Effort **S**.
  - <!-- 2026-07-17 --> **AEO evaluation done** (plan: browser artifact). The whole public answer-set for "who owns rental housing" is stale 2018 RHFS (CRS R47332); our 2024 cut is fresh and 3 headline claims (LLCs>individuals by units, ~63% free-and-clear, size split) are **uncontested**. Winnable AEO targets, ranked: (1) individual-vs-LLC / % units owned by LLCs, (2) % rentals free-and-clear (a vacuum — current answers use the wrong owner-occupied universe), (3) "who owns rental property in America" head term, (4) "are individuals leaving" as a stock-vs-flow reframe. Deprioritize "corporate/institutional ownership" (locked by high-DR incumbents). Watch-out: Chandan Economics reached 2024 first with an SFR "individuals dominate" framing — our moat is the properties-vs-units + SFR-vs-all-rentals clarity, now baked into the report (new FAQ + on-page disambiguation).
  - <!-- 2026-07-17 --> **Conversion — report-contextual capture SHIPPED** on `/reports/rental-ownership`: reports previously had NO contextual lead capture (only the global exit/sticky Deal-Analysis-Checklist). Added a source-tagged "Get the next data report" `NewsletterSignup` (`source="report-rental-ownership"` → measurable report→lead) after the properties-vs-units section; CSV deliberately **kept ungated** (free-to-cite is what earns the links). **Next:** roll the same capture to the other 4 reports once proven; internal links into the report from high-traffic posts/calculators (phased).
